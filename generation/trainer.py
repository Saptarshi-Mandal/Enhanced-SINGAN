...
from models.modules.losses import TotalVariationLoss
...
class Trainer():
    ...
    def _init_optim(self):
        ...
        self.variational_loss = TotalVariationLoss(tv_weight=self.args.total_variational_weight)
    ...
    def _init_local(self, loader):    
        ...
        # parameters
        self.losses = {'D': [], 'D_r': [], 'D_gp': [], 'D_f': [], 'G': [], 'G_recon': [], 'G_adv': [], 'TV': []}
    ...
    def _generator_iteration(self, noises, reals, amps, generated_data_adv):
        ...
        if self.args.adversarial_weight:
            self.total_variational_loss = self.variational_loss(generated_data_rec)
            loss+= self.total_variational_loss
            self.losses['TV'].append(self.total_variational_loss.item())
    ...
    def _train_iteration(self, loader):
        ...
        if self.step % self.args.print_every == 0:
            ...
            # print(self.losses['TV'][-1], type(self.losses['TV'][-1]))
            line2print += ', TV: {:.5f}'.format(self.losses['TV'][-1])
